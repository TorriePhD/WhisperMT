...
import torch
from trainSingle import SingleDataset

class selfTrainDataset(torch.utils.data.Dataset):
    def __init__(self,translatePrompt=False,sanity_check=False,test=False):
        self.translatePrompt = translatePrompt
        datasetPath = Path("/home/st392/fsl_groups/grp_nlp/compute/mt/selfTrainQE")
        textFiles = list(datasetPath.rglob("*.txt"))
        self.sanity_check = sanity_check
        self.startTrascriptToken = 50258
        wavFiles = list(datasetPath.rglob("*.wav"))
        self.test = test
        self.textData, self.wavData = None,None
        self.loadDataset(textFiles, wavFiles)
        
    def loadDataset(self, textFiles, wavFiles):
        self.wavData = {}
        self.textData = {}
        textfileNames = []
        for file in tqdm(textFiles):
            langauge1 = file.parent.parent.stem
            language2 = file.parent.stem
            if self.sanity_check and (langauge1 != "spanish" or language2 != "arabic"):
                continue
            if langauge1 =="marathi" or language2 == "marathi":
                continue
            if not self.test and (langauge1 == "french" or language2 == "french" or langauge1 == "dutch" or language2 == "dutch" ):
                continue
            elif self.test:
                if langauge1 == "french" and language2 != langauge1:
                    pass
                else:
                    continue
            ...
            if self.translatePrompt and langauge1 != language2:
                #translation prompt for language2
                forced_decoder_ids = processor.get_decoder_prompt_ids(language=langauge1, task="translate")    
            ...
            elif self.translatePrompt and langauge1 == language2:
                #translation prompt for language2
                forced_decoder_ids = processor.get_decoder_prompt_ids(language=langauge1, task="transcribe")
            ...
            if tokenizedText["input_ids"].shape[1]+len(forced_decoder_ids) > 448:
                continue
            #decode the prompt
            self.textData[file.stem+"*"+langauge1+"*"+language2] = {"tokenizedText": tokenizedText, "forced_decoder_ids": forced_decoder_ids}
            textfileNames.append(file.stem)
        #remove files that don't have a corresponding text file
        wavFiles = [file for file in wavFiles if file.stem in textfileNames]
        for file in tqdm(wavFiles):
            ...
            self.wavData[file.stem] = audio
        #check if there are any text file that don't have a corresponding wav file
    ...
    def __getitem__(self, idx):
        ...
        wavKey = key.split("*")[0]
        input_features = self.wavData[wavKey].squeeze(0)
...
if __name__ == "__main__":
    ...
    dataset = selfTrainDataset(translatePrompt=translatePrompt, sanity_check=False)
    data_collator = DataCollatorSpeechSeq2SeqWithPadding(processor=processor)
    model.generation_config.language = "hindi"
    model.generation_config.task = "transcribe"
    baseOutputDir = Path("/home/st392/fsl_groups/grp_nlp/compute/mt/resultsNoMarathi/")
